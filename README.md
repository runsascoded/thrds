...
slck push [-r <remote>]                # commit, then sync every thread to a staging-role remote (refuses if it moved; -f)
...
**`fetch` gives `diff` a merge base.** A Slack channel keeps no version of itself, so `slck fetch` keeps one for it: `refs/remotes/{staging,prod}` plus the computed merge base `refs/heads/upstream` in the session repo, each pointing at a commit whose tree is what Slack last projected — moving only when the content actually moved, touching nothing else (`-n` previews; `slck fetch staging` reads just that remote, the skipped one feeding the composite from its stored ref). That turns `diff`'s two-way delta into a classification: a hunk is *changed in Slack* (`pull` applies it), *changed locally* (`push` sends it, and the diff flips to read slack → local), or *CONFLICT* (both moved since the last fetch) — distinctions a two-way diff structurally can't make, since "Slack edited this" and "`pull` is about to eat your edit" look identical. The refs are also queryable directly: `git show upstream` is what changed on Slack since you last looked, `git diff upstream HEAD` is what you've changed that Slack hasn't seen, and `git diff --diff-filter=M staging prod` is whether a posted thread's frozen staged copy has drifted from what's live.
