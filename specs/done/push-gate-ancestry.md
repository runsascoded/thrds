...
**Correction (same day):** this section first called the bootstrap case "the remaining hole in this gate". It isn't one. With no base recorded the gate returns early and `push` proceeds — but that is the *documented* "local wins" resolution, the one `_fetch_refs` names on stderr when it withholds an unconfirmed composite, and ghpr lets a bootstrap push through for exactly the same reason. The two adapters already agree; item 3 needed a test, not code.

What did need pinning is the interaction between the two features: an observation that disagrees with local must not become the gate's base, or the next push would believe it had already taken the remote's side. `test_ambiguous_bootstrap_withholds_the_composite` now asserts that, one level down from the composite claim it was already making.

**Actually still open:** sessions fetched before base refs existed. Splitting the refs left their first post-upgrade push ungated, since "no base" reads as bootstrap. Fixed by `tracking.seed_base_refs` — a one-time adoption of each observation ref as its base, guarded by a `thrds.baseRefsSeeded` config flag rather than by the base's absence, because absence is also how a diverged fetch legitimately leaves things.
